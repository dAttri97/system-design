...
```bash
mvn -q compile exec:java -Dexec.mainClass=pop.machine.coding.BookingSystemDemo
```
...
```bash
javac -d /tmp/booking $(find src/main/java/pop -name '*.java') && java -cp /tmp/booking pop.machine.coding.BookingSystemDemo
```
